...
from model.simple_fss import FewShotViTModel
...
def validate(val_loader, model, criterion):
    ...
    if main_process():
        logger.info(
            "FBIoU---Val result: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.".format(
                mIoU, mAcc, allAcc
            )
        )
        for i in range(args.classes):
