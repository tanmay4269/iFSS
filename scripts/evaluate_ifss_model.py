...
def main():
    ...
    assert (
        not args.iou_analysis if not single_model_eval else True
    ...
    for dataset_name in args.datasets.split(","):
        ...
        for checkpoint_path in checkpoints_list:
            ...
            vis_callback = (
                get_prediction_vis_callback(logs_path, dataset_name, args.thresh)
                if args.vis_preds
                else None
            )
...
def get_checkpoints_list_and_logs_path(args, cfg):
    ...
    if args.exp_path:
        ...
        checkpoints_list = [
            Path(utils.find_checkpoint(cfg.INTERACTIVE_MODELS_PATH, args.checkpoint))
        ]
...
def save_results(
    ...
    noc_list, over_max_list = utils.compute_noc_metric(
        q_ious, iou_thrs=iou_thrs, max_clicks=args.n_clicks
    )
    s_noc_list, s_over_max_list = utils.compute_noc_metric(
        s_ious, iou_thrs=iou_thrs, max_clicks=args.n_clicks
    )

    row_name = "last" if row_name == "last_checkpoint" else row_name
    model_name = (
        str(logs_path.relative_to(args.logs_path)) + ":" + logs_prefix
        if logs_prefix
        else logs_path.stem
    )
    ...
    s_header, s_table_row = utils.get_results_table(
        s_noc_list,
        s_over_max_list,
        row_name,
        dataset_name,
    ...
    if args.print_ious:
        ...
        if target_iou_int not in [80, 85, 90]:
            noc_list, over_max_list = utils.compute_noc_metric(
                q_ious, iou_thrs=[args.target_iou], max_clicks=args.n_clicks
            )
            table_row += f" NoC@{args.target_iou:.1%} = {noc_list[0]:.2f};"
            table_row += (
                f" >={args.n_clicks}@{args.target_iou:.1%} = {over_max_list[0]}"
            )
    ...
    log_path = (
        logs_path / f"{name_prefix}{args.eval_mode}_{args.mode}_{args.n_clicks}.txt"
    )
    if log_path.exists():
...
def save_iou_analysis_data(
    args, dataset_name, logs_path, logs_prefix, dataset_results, model_name=None
):
    ...
    pkl_path = (
        logs_path
        / f"plots/{name_prefix}{args.eval_mode}_{args.mode}_{args.n_clicks}.pickle"
    )
...
def get_prediction_vis_callback(logs_path, dataset_name, prob_thresh):
    ...
    def callback(
        ...
        support_image_with_mask = draw_with_blend_and_clicks(
            support_image * 255,
            support_pred_probs > prob_thresh,
            clicks_list=clicks_list,
        )
        query_image_with_mask = draw_with_blend_and_clicks(
            query_image * 255, query_pred_probs > prob_thresh
        )
