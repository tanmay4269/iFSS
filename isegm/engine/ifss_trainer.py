...
class iFSSTrainer(object):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            with torch.no_grad():
                ...
                for click_indx in range(num_iters):
                    ...
                    s_points = get_next_points(prev_s_output, orig_s_gt_mask, s_points, click_indx + 1)

                    if not validation:
                        self.net.train()

                if self.net.with_prev_mask and self.prev_mask_drop_prob > 0 and last_click_indx is not None:
                    zero_mask = np.random.random(size=prev_s_output.size(0)) < self.prev_mask_drop_prob
                    prev_s_output[zero_mask] = torch.zeros_like(prev_s_output[zero_mask])
            ...
            if self.is_master:
                with torch.no_grad():
                    for m in metrics:
                        m.update(*(outputs.get(x) for x in m.pred_outputs),
    ...
    def save_visualization(self, splitted_batch_data, outputs, global_step, prefix):
        ...
        predicted_s_instance_masks = torch.sigmoid(outputs['s_instances']).detach().cpu().numpy()
        ...
        s_image_with_points = draw_points(s_image, s_points[:self.max_interactive_points], (0, 255, 0))
        s_image_with_points = draw_points(s_image_with_points, s_points[self.max_interactive_points:], (0, 0, 255))

        s_gt_mask[s_gt_mask < 0] = 0.25
        s_gt_mask = draw_probmap(s_gt_mask)
        s_predicted_mask = draw_probmap(s_predicted_mask)
        s_viz_image = np.hstack((s_image_with_points, s_gt_mask, s_predicted_mask)).astype(np.uint8)
