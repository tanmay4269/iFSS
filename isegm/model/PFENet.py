...
if __name__ == "__main__":
    model = PFENet(layers=50, classes=2, zoom_factor=8, pretrained=False, vgg=False)
    input_tensor = torch.randn(1, 3, 473, 473).cuda()
    support_tensor = torch.randn(1, 1, 3, 473, 473).cuda()
    support_mask = torch.ones(1, 1, 473, 473).cuda()
    output = model(input_tensor, support_tensor, support_mask)
    print(output)
