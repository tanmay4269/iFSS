...
class BasePredictor(object):
    ...
    def set_input_images(self, query_image, support_image):
        ...
        self.prev_query_prediction = torch.zeros_like(
            self.original_query_image[:, :1, :, :]
        )
        ...
        self.prev_support_prediction = torch.zeros_like(
            self.original_support_image[:, :1, :, :]
        )

    def get_prediction(self, clicker, prev_query_mask=None, prev_support_mask=None):
        support, query = edict(), edict()
        support.points = clicker.get_clicks()

        # Gets the right model when needed
        if self.click_models is not None:
            model_indx = (
                min(
                    clicker.click_indx_offset + len(support.points),
        ...
        query.image, _, _ = self.apply_transforms(input_query_image, [support.points])
        ...
        support.image, support.points, is_image_changed = self.apply_transforms(
            input_support_image, [support.points]
        )

        # Prep's prev output
        support.prev_output = torch.tensor(
            prev_support_mask, dtype=torch.float32, device=support.image.device
        ).view(1, 1, *prev_support_mask.shape)
        query.prev_output = torch.tensor(
            prev_query_mask, dtype=torch.float32, device=query.image.device
        ).view(1, 1, *prev_query_mask.shape)

        query_pred_logits, support_pred_logits = self._get_prediction(
            support, query, is_image_changed
        )
        ...
        return (
            query_prediction.cpu().numpy()[0, 0],
            support_prediction.cpu().numpy()[0, 0],
        )

    def _get_prediction(self, support, query, is_image_changed):
        support.points = self.get_points_nd(support.points)
        outputs = self.net(support, query, pretraining_enabled=False)
