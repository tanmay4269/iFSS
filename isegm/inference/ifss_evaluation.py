...
def evaluate_dataset(dataset, predictor, **kwargs):
    ...
    for index in tqdm(range(len(dataset)), leave=False):
        ...
        sample_ious = evaluate_sample(
            query_sample.image,
            query_sample._encoded_masks[..., 0],
            support_sample.image,
            support_sample.gt_mask,
            predictor,
            sample_id=index,
            **kwargs
        )
...
def evaluate_sample(
    ...
    Notes:
        ...
        - The prev output being fed is thresholded
    
    TODOs:
        - [ ] keep the convention support before query. it just confuses coz in training, 
            support is before query
    """
