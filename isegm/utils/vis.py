...
class UndoNormalize(A.ImageOnlyTransform):
    """Undo the normalization applied by Albumentations' Normalize class."""

    def __init__(self, mean, std, max_pixel_value=255.0, always_apply=True, p=1.0):
    ...
    def apply(self, img, **params):
        """Undo normalization formula."""
        mean = np.array(self.mean).reshape(-1, 1, 1) * self.max_pixel_value
        std = np.array(self.std).reshape(-1, 1, 1) * self.max_pixel_value
        img = img * std + mean
        img = np.clip(img / self.max_pixel_value, 0, 1)
        return img
